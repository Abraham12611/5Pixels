...
function getDefaultSize(
  sizes: OutputSizeOption[] | undefined
): OutputSizeOption {
...
export function TestLabForm({ product, initialBalance }: TestLabFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canAfford) {
      ...
      setError(
        `This preset costs ${cost} credits. Your balance is ${initialBalance}. Grant yourself credits on the support page if needed.`
      );
      return;
    }
    ...
    try {
      ...
      const idempotencyKey = `admin-test:${product.id}:${uuidv4()}`;
      ...
      // On success the server action redirects to the consumer result page.
    } catch (err) {
      if (
      ...
      <section className="border-cream-100/10 bg-charcoal-850 rounded-2xl border p-6">
        ...
        <div className="mt-4">
          ...
          <Select
            ...
            onChange={(e) => {
              const next = outputSizes.find((s) => sizeKey(s) === e.target.value);
      ...
      <section className="border-cream-100/10 bg-charcoal-850 rounded-2xl border p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button
            ...
            className="w-full sm:w-auto"
          >
            {loading ? progress || "Testing…" : "Run test generation"}
          </Button>
        </div>
        {isPoster && (
